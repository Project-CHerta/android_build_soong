...
def parse_args():
  ...
  config["CHertaDesc"] = config["BuildDesc"]
  config["CHertaDevice"] = config["DeviceName"]
...
def generate_build_info(args):
  ...
  if config["BuildVariant"] == "user":
    ...
    print(f"ro.build.display.id?={config['CHertaDesc']}")
  ...
  print(f"ro.cherta.device={config['CHertaDevice']}")
